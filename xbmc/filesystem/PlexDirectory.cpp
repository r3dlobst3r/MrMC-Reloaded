...
bool CPlexDirectory::GetDirectory(const CURL& url, CFileItemList &items)
{
  ...
  CLog::Log(LOGDEBUG, "CPlexDirectory::GetDirectory strURL = {}", strUrl);

  CVideoDatabase vdatabase;
  vdatabase.Open();
  bool hasMovies = vdatabase.HasContent(VideoDbContentType::MOVIES);
  bool hasShows = vdatabase.HasContent(VideoDbContentType::TVSHOWS);
  vdatabase.Close();
  ...
  if (StringUtils::StartsWithNoCase(strUrl, "plex://movies/"))
  {
    ...
    else
    {
      ...
      {
        CLog::Log(LOGDEBUG, "CPlexDirectory::GetDirectory no client or client not present {}", CURL::GetRedacted(strUrl));
        return false;
      }
      ...
      CLog::Log(LOGDEBUG, "CPlexDirectory::GetDirectory' client({}), found {} movies", client->GetServerName(), items.Size());
    }
    return true;
  }
...
//        items.ClearSortState();
      ...
      CLog::Log(LOGDEBUG, "CPlexDirectory::GetDirectory' client({}), found {} shows", client->GetServerName(), items.Size());
    }
    return true;
  }
  ...
  {
    CLog::Log(LOGDEBUG, "CPlexDirectory::GetDirectory got nothing from {}", CURL::GetRedacted(strUrl));
  }
