...
, m_hasClients(false)
{
  // MrMC note: Kodi 21 removed the CURL static redaction registry.
...
void CEmbyServices::Stop()
{
  ...
  std::unique_lock<CCriticalSection> lock2(m_clients_lock);
  m_clients.clear();
  m_playState = MediaServicesPlayerState::stopped;
  m_hasClients = false;
}
...
{
  return (!CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_EMBYACESSTOKEN).empty());
}
...
{
  std::unique_lock<CCriticalSection> lock(m_clients_lock);
  clients = m_clients;
}
...
void CEmbyServices::OnSettingAction(const std::shared_ptr<const CSetting>& setting)
{
  if (setting == nullptr)
    return;
  ...
  {
    if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_EMBYSIGNIN) == strSignIn)
    {
      ...
      if (CGUIDialogNetworkSetup::ShowAndGetNetworkAddress(path, true))
      {
        ...
        if (!curl2.GetHostName().empty() && !curl2.GetUserName().empty())
        {
          if (AuthenticateByName(curl2))
          {
            ...
            CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_EMBYSIGNIN, strSignOut);
    ...
    else
    {
      ...
      m_userId.clear();
      m_accessToken.clear();
      CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_EMBYSIGNIN, strSignIn);
  ...
  else if (settingId == CSettings::SETTING_SERVICES_EMBYSERVERSOURCES)
  {
    ...
    embySavedSources = CServiceBroker::GetSettingsComponent()->GetSettings()->GetList(CSettings::SETTING_SERVICES_EMBYSAVEDSOURCES);
    ...
    CServiceBroker::GetSettingsComponent()->GetSettings()->SetList(CSettings::SETTING_SERVICES_EMBYSAVEDSOURCES, embySavedSources);

    if (embySavedSources.size() == serverInfos.size())
      CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_EMBYSERVERSOURCES, "All Sources");
    else if (embySavedSources.size() == 0)
      CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_EMBYSERVERSOURCES, "None");
    else
      CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_EMBYSERVERSOURCES, "Selected Sources");
...
void CEmbyServices::InitiateSignIn()
{
  ...
  if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_EMBYSIGNINPIN) == strSignIn)
  {
    ...
    {
      // change prompt to 'sign-out'
      CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_EMBYSIGNINPIN, strSignOut);
  ...
  else
  {
    ...
    m_userId.clear();
    m_accessToken.clear();
    CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_EMBYSIGNINPIN, strSignIn);
    CLog::Log(LOGDEBUG, "CEmbyServices:OnSettingAction sign-out ok");
...
void CEmbyServices::Announce(AnnouncementFlag flag, const std::string& sender, const std::string& message, const CVariant& data)
{
  ...
  else if ((flag & AnnouncementFlag::Other) && sender == "emby")
  {
...
void CEmbyServices::OnSettingChanged(const std::shared_ptr<const CSetting>& setting)
{
...
{
  CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_EMBYUSERID, m_userId);
  CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_EMBYSERVERURL, m_serverURL);
  CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_EMBYACESSTOKEN, m_accessToken);
  CServiceBroker::GetSettingsComponent()->GetSettings()->Save();
}

void CEmbyServices::GetUserSettings()
{
  m_userId = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_EMBYUSERID);
  m_serverURL  = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_EMBYSERVERURL);
  m_accessToken = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_EMBYACESSTOKEN);
}
...
void CEmbyServices::Process()
{
  ...
  signInByManual = (CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_EMBYSIGNIN) == strSignOut);
  ...
  while (!m_bStop)
  {
    ...
    if (signInByPin && CServiceBroker::GetNetwork().IsConnected())
    {
      std::string ip;
      if (CDNSNameCache::Lookup("connect.mediabrowser.tv", ip))
      {
        in_addr_t embydotcom = inet_addr(ip.c_str());
        if (CServiceBroker::GetNetwork().PingHost(embydotcom, 0, 1000))
        {
...
bool CEmbyServices::AuthenticateByName(const CURL& url)
{
  ...
  CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_GENERAL_SERVER_TYPE,"emby");
  CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_GENERAL_SERVER_UUID,"");
  CServiceBroker::GetSettingsComponent()->GetSettings()->Save();
...
bool CEmbyServices::PostSignInPinCode()
{
  ...
  CVariant data;
  data["deviceId"] = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_UUID);
  std::string jsonBody;
...
bool CEmbyServices::GetSignInByPinReply()
{
  ...
  curl.SetOption("deviceId", CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_UUID));
...
EmbyServerInfoVector CEmbyServices::GetConnectServerList(const std::string &connectUserId, const std::string &connectAccessToken)
{
  ...
  CGUIDialogBusy *busyDialog = nullptr; // MrMC: busy dialog auto-handled via CGUIDialogBusy::Wait in modern flows
