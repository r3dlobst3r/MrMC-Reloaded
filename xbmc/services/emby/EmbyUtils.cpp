...
void CEmbyUtils::PrepareApiCall(const std::string& userId, const std::string& accessToken, XFILE::CCurlFile &curl)
{
  ...
  curl.SetRequestHeader(EmbyAuthorizationHeader,
    StringUtils::Format("MediaBrowser Client=\"%s\", Device=\"%s\", DeviceId=\"%s\", Version=\"%s\", UserId=\"%s\"",
      ...
      CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_UUID).c_str(),
...
bool CEmbyUtils::DeleteEmbyMedia(CFileItem &item)
{
  ...
  emby.SetCustomRequest("DELETE");
  emby.Get(curl.Get(), response);
  return true;
}
...
bool CEmbyUtils::GetAllEmbyInProgress(CFileItemList &items, bool tvShow)
{
  ...
  {
    CFileItemList embyItems;
    int limitTo = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_EMBYLIMITHOMETO);
    if (limitTo < 2)
      return false;
...
bool CEmbyUtils::ShowMusicInfo(CFileItem item)
{
  ...
  {
    // MrMC: artist info dialog requires a db-backed artist id
  }
  return true;
}
...
bool CEmbyUtils::ParseEmbySeries(CFileItemList &items, const CURL &url, const CVariant &variant)
{
  ...
  if (!variant.isNull() || variant.isObject() || variant.isMember("Items"))
  {
    ...
    for (auto variantItemIt = variantItems.begin_array(); variantItemIt != variantItems.end_array(); ++variantItemIt)
    {
      ...
      newItem->GetVideoInfoTag()->SetPlayCount(static_cast<int>(item["UserData"]["PlayCount"].asInteger()));
      if (newItem->GetVideoInfoTag()->GetPlayCount() > 0)
        newItem->SetOverlayImage(CGUIListItem::ICON_OVERLAY_UNWATCHED);

      newItem->GetVideoInfoTag()->SetTitle(title);
      ...
      newItem->GetVideoInfoTag()->SetPlayCount((int)watchedEpisodes >= newItem->GetVideoInfoTag()->m_iEpisode);
...
bool CEmbyUtils::ParseEmbySeasons(CFileItemList &items, const CURL &url, const CVariant &series, const CVariant &variant)
{
  ...
  for (auto variantItemIt = variantItems.begin_array(); variantItemIt != variantItems.end_array(); ++variantItemIt)
  {
    ...
    newItem->GetVideoInfoTag()->SetTitle(item["Name"].asString());
    ...
    newItem->GetVideoInfoTag()->m_iEpisode = totalEpisodes;
    newItem->GetVideoInfoTag()->SetPlayCount((totalEpisodes == watchedEpisodes) ? 1 : 0);
    ...
    if ((newItem->GetVideoInfoTag()->GetPlayCount() > 0) && (newItem->GetVideoInfoTag()->m_iEpisode > 0))
      newItem->SetOverlayImage(CGUIListItem::ICON_OVERLAY_UNWATCHED);
  ...
  if (!items.IsEmpty())
  {
    ...
    { // flatten if one season or flatten always
      ...
      XFILE::CDirectory::GetDirectory("emby://tvshows/seasons/" + Base64URL::Encode(curl.Get()), tempItems, XFILE::CDirectory::CHints());
...
bool CEmbyUtils::ParseEmbyAudio(CFileItemList &items, const CURL &url, const CVariant &variant)
{
  ...
  for (auto variantItemIt = variantItems.begin_array(); variantItemIt != variantItems.end_array(); ++variantItemIt)
  {
    ...
    embyItem->GetMusicInfoTag()->SetType(MediaTypeSong);
    ...
    { CDateTime dt; dt.SetFromW3CDateTime(item["DateCreated"].asString()); embyItem->GetMusicInfoTag()->SetDateAdded(dt); }
    { CDateTime dt; dt.SetFromW3CDateTime(item["LastPlayedDate"].asString()); embyItem->GetMusicInfoTag()->SetLastPlayed(dt); }
...
bool CEmbyUtils::ParseEmbyAlbum(CFileItemList &items, const CURL &url, const CVariant &variant)
{
  ...
  for (auto variantItemIt = variantItems.begin_array(); variantItemIt != variantItems.end_array(); ++variantItemIt)
  {
    ...
    embyItem->GetMusicInfoTag()->SetType(MediaTypeAlbum);
    embyItem->GetMusicInfoTag()->SetTitle(item["Name"].asString());
...
bool CEmbyUtils::ParseEmbyArtists(CFileItemList &items, const CURL &url, const CVariant &variant)
{
  ...
  for (auto variantItemIt = variantItems.begin_array(); variantItemIt != variantItems.end_array(); ++variantItemIt)
  {
    ...
    embyItem->GetMusicInfoTag()->SetType(MediaTypeArtist);
    embyItem->GetMusicInfoTag()->SetTitle(item["Name"].asString());
  ...
  items.SetProperty("library.filter", "true");
  items.GetMusicInfoTag()->SetType(MediaTypeArtist);
  items.SetCacheToDisc(CFileItemList::CACHE_NEVER);
...
CFileItemPtr CEmbyUtils::ToVideoFileItemPtr(CURL url, const CVariant &variant, std::string type)
{
  ...
  {
    CBookmark resume;
    resume.totalTimeInSeconds = item->GetVideoInfoTag()->m_duration;
    resume.timeInSeconds = static_cast<int>(TicksToSeconds(variant["UserData"]["PlaybackPositionTicks"].asUnsignedInteger()));
    item->GetVideoInfoTag()->SetResumePoint(resume);
  }
  ...
  if (variant["UserData"]["PlayCount"].asInteger() && item->GetVideoInfoTag()->GetResumePoint().timeInSeconds <= 0)
  {
    item->GetVideoInfoTag()->SetPlayCount(static_cast<int>(variant["UserData"]["PlayCount"].asInteger()));
  }
