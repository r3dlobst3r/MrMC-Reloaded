...
CEmbyClient::~CEmbyClient()
{
  delete m_clientSync; m_clientSync = nullptr;
}
...
bool CEmbyClient::Init(const EmbyServerInfo &serverInfo)
{
  ...
  m_clientSync = new CEmbyClientSync(m_serverInfo.ServerName, m_serverInfo.ServerURL,
    CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_UUID).c_str(), serverInfo.AccessToken);
  m_clientSync->Start();
...
void CEmbyClient::SetWatched(CFileItem &item)
{
  ...
  if (item.IsVideo())
    ...
    lastPlayed = item.GetMusicInfoTag()->GetLastPlayed();
  else
    lastPlayed = CDateTime::GetUTCDateTime();

  if (content == "movies")
  {
    std::unique_lock<CCriticalSection> lock(m_viewMoviesLock);
    for (auto &view : m_viewMovies)
    {
      bool hit = view->SetWatched(itemId,
        item.GetVideoInfoTag()->GetPlayCount(), item.GetVideoInfoTag()->GetResumePoint().timeInSeconds);
      if (hit)
        break;
    }
  }
  ...
  {
    std::unique_lock<CCriticalSection> lock(m_viewTVShowsLock);
...
void CEmbyClient::SetUnWatched(CFileItem &item)
{
  ...
  XFILE::CCurlFile curlfile;
  curlfile.SetCustomRequest("DELETE");
...
bool CEmbyClient::GetMovies(CFileItemList &items, std::string url, bool fromfilter)
{
  ...
  if (fromfilter)
  {
    std::unique_lock<CCriticalSection> lock(m_viewMoviesFilterLock);
...
bool CEmbyClient::GetTVShows(CFileItemList &items, std::string url, bool fromfilter)
{
  ...
  if (fromfilter)
  {
    std::unique_lock<CCriticalSection> lock(m_viewTVShowsFilterLock);
...
bool CEmbyClient::GetMusicArtists(CFileItemList &items, std::string url)
{
  ...
  std::unique_lock<CCriticalSection> lock(m_viewMusicLock);
...
void CEmbyClient::UpdateViewItems(const std::vector<std::string> &ids)
{
  ...
  for (auto variantItemIt = variantItems.begin_array(); variantItemIt != variantItems.end_array(); ++variantItemIt)
  {
    ...
    if (item != nullptr)
    {
      ...
      if (!thumb.empty() && CServiceBroker::GetTextureCache()->HasCachedImage(thumb))
      {
        updateArtwork = true;
      ...
      if (!banner.empty() && CServiceBroker::GetTextureCache()->HasCachedImage(banner))
      {
        updateArtwork = true;
        CServiceBroker::GetTextureCache()->ClearCachedImage(banner);
      }

      std::string fanart = item->GetArt("fanart");
      if (!fanart.empty() && CServiceBroker::GetTextureCache()->HasCachedImage(fanart))
      {
        updateArtwork = true;
        CServiceBroker::GetTextureCache()->ClearCachedImage(fanart);
      }
...
{
  std::unique_lock<CCriticalSection> lock(m_topViewIDsLock);
  std::unique_lock<CCriticalSection> movielock(m_viewMoviesLock);
  std::unique_lock<CCriticalSection> tvlock(m_viewMoviesLock);
  std::unique_lock<CCriticalSection> musiclock(m_viewMoviesLock);
...
{
  std::unique_lock<CCriticalSection> lock(m_viewPhotosLock);
  std::vector<EmbyViewInfo> infos;
  ...
  // channels,
    ...
    for (auto viewIt = viewsObject.begin_array(); viewIt != viewsObject.end_array(); ++viewIt)
    {
      ...
      if (!std::any_of(mediaTypes.cbegin(), mediaTypes.cend(),
          ...
          {
            return CMediaTypes::IsMediaType(type, mediaType);
          }))
        continue;
