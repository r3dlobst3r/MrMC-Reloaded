...
void CServicesManager::Announce(AnnouncementFlag flag, const std::string& sender, const std::string& message, const CVariant& data)
{
     ...
     switch(mkhash(message.c_str()))
      {
        ...
        case "OnStop"_mkhash:
        {
          ...
          if (!msg.empty())
          {
            ...
            AddJob(new CServicesManagerJob(g_application.CurrentFileItem(), item.GetVideoInfoTag()->GetResumePoint().timeInSeconds, msg));
          }
          break;
        }
        default:
  ...
  else if ((flag & AnnouncementFlag::Other) && sender == "mrmc")
  {
    if (message == "ReloadSkin")
    {
      ReloadProfiles();
    }
  }
...
bool CServicesManager::UpdateMediaServicesLibraries(const CFileItem &item)
{
  if (item.HasProperty("MediaServicesItem"))
  {
    ...
    if (item.HasProperty("PlexItem"))
      CServiceBroker::GetAnnouncementManager()->Announce(ANNOUNCEMENT::Other, std::string("plex"), "UpdateLibrary", data);
    if (item.HasProperty("EmbyItem"))
      CServiceBroker::GetAnnouncementManager()->Announce(ANNOUNCEMENT::Other, std::string("emby"), "UpdateLibrary", data);
  }
  return true;
}
...
{
  if (CPlexUtils::HasClients())
    CServiceBroker::GetAnnouncementManager()->Announce(ANNOUNCEMENT::Other, std::string("plex"), std::string("ReloadProfiles"));
  if (CEmbyUtils::HasClients())
    CServiceBroker::GetAnnouncementManager()->Announce(ANNOUNCEMENT::Other, std::string("emby"), std::string("ReloadProfiles"));
  return true;
}
...
void CServicesManager::RegisterMediaServicesHandler(IMediaServicesHandler *mediaServicesHandler)
{
  ...
  std::unique_lock<CSharedSection> lock(m_mediaServicesCritical);
