...
void CPlexUtils::GetDefaultHeaders(XFILE::CCurlFile *curl)
{
  ...
  curl->SetRequestHeader("X-Plex-Client-Identifier", CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_UUID));
...
void CPlexUtils::ReportProgress(CFileItem &item, double currentSeconds)
{
  ...
  if ((g_playbackState == MediaServicesPlayerState::stopped || g_progressSec == 0 || g_progressSec > 120))
  {
    ...
    if (!status.empty())
    {
      ...
      std::string id    = item.GetMediaServiceId();
      int totalSeconds  = item.GetVideoInfoTag()->GetResumePoint().totalTimeInSeconds;
...
//  library/metadata/
  ...
  std::string response;
  plex.SetSilent(true);
  plex.SetCustomRequest("DELETE");
  plex.Get(url2.Get(), response);
  return true;
}
  ...
  if (!variant.isNull() && variant.isObject() && variant.isMember("MediaContainer"))
  {
    if (!watched)
    {
      ...
      for (int i = 0; i < tempItems.Size() && i <= limit/2; ++i)
      {
        ...
        if (temp->GetVideoInfoTag()->GetPlayCount() > 0)
          continue;
        items.Add(temp);
      }
    }
...
bool CPlexUtils::GetAllPlexRecentlyAddedMoviesAndShows(CFileItemList &items, bool tvShow, bool unWatched)
{
  ...
  if (CPlexServices::GetInstance().HasClients())
  {
    ...
    int limitTo = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_PLEXLIMITHOMETO);
    if (limitTo < 2)
      return false;
          ...
          for (auto variantIt = variantHub.begin_array(); variantIt != variantHub.end_array(); ++variantIt)
          {
            ...
            if (hub.isMember("type"))
            {
              ...
              if (type == "show" && size > 0)
              {
                ...
                {
                  std::string label = plexShow[i]->GetVideoInfoTag()->GetTitle() + " (" +  plexShow[i]->GetVideoInfoTag()->m_strShowTitle + ")";
              ...
              else if (type == "movie"  && size > 0)
              {
                ...
                {
                  std::string label = plexMovies[i]->GetVideoInfoTag()->GetTitle();
...
{
  if (!CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_SERVICES_PLEXTRANSCODE))
    return true;
  ...
  bool transcode = true;
  if ((!CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_SERVICES_PLEXTRANSCODELOCAL)) &&
        CPlexServices::GetInstance().ClientIsLocal(cleanUrl))
  {
    transcode = false;
    /// transcode h265 or VC1 if user selected it
    if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_SERVICES_PLEXTRANSCODELOCALEXCLUSION) &&
          item.GetVideoInfoTag()->m_streamDetails.GetVideoCodec() == "vc1")
        transcode = true;
    
    if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_SERVICES_PLEXTRANSCODELOCALEXCLUSIONHEVC) &&
  ...
  int res = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_PLEXQUALITY);
  ...
  */
  ...
  if (!CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_SERVICES_PLEXTRANSCODEAUDIO))
  {
...
bool CPlexUtils::ShowMusicInfo(CFileItem item)
{
  ...
  {
    CGUIDialogSongInfo *dialog = (CGUIDialogSongInfo *)CServiceBroker::GetGUI()->GetWindowManager().GetWindow(WINDOW_DIALOG_SONG_INFO);
    if (dialog)
    {
      dialog->SetSong(&item);
      dialog->Open();
  ...
  {
    CGUIDialogMusicInfo::ShowForAlbum(item.GetMusicInfoTag()->GetAlbumId());
  }
  else if (type == MediaTypeArtist)
  {
    // MrMC: artist info dialog requires a db-backed artist id, not available for plex users
    CLog::Log(LOGDEBUG, "CPlexUtils: artist info dialog not available for plex user items");
  }
  return true;
}
...
//    items.SetProperty("library.filter", "true");
    items.GetMusicInfoTag()->SetType(MediaTypeAlbum);
    return rtn;
  }
  return true;
}
...
bool CPlexUtils::ParsePlexVideos(CFileItemList &items, CURL url, const CVariant &videos, std::string type, bool formatLabel, int season /* = -1 */)
{
  ...
  for (auto variantIt = variantVideo.begin_array(); variantIt != variantVideo.end_array(); ++variantIt)
  {
    ...
    if (season > -1)
    {
      ...
      plexItem->SetArt("icon", imagePath);
    ...
    plexItem->GetVideoInfoTag()->SetTitle(title);
    ...
    if (item.isMember("lastViewedAt") && !item.isMember("viewOffset"))
      plexItem->GetVideoInfoTag()->SetPlayCount(1);

    if (plexItem->HasVideoInfoTag() && plexItem->GetVideoInfoTag()->GetPlayCount() > 0)
      plexItem->SetOverlayImage(CGUIListItem::ICON_OVERLAY_UNWATCHED);
    ...
    plexItem->GetVideoInfoTag()->SetResumePoint(m_bookmark);
    plexItem->SetStartOffset(item["viewOffset"].asInteger() / 1000);
    ...
    if (formatLabel)
    {
      ...
      plexItem->SetLabel(plexItem->GetLabel());
    }
...
bool CPlexUtils::ParsePlexSeries(CFileItemList &items, const CURL &url, const CVariant &directory)
{
  ...
  for (auto variantIt = variantDirectory.begin_array(); variantIt != variantDirectory.end_array(); ++variantIt)
  {
    ...
    plexItem->GetVideoInfoTag()->SetTitle(item["title"].asString());
    plexItem->GetVideoInfoTag()->SetPlotOutline(item["tagline"].asString());
    plexItem->GetVideoInfoTag()->SetPlot(item["summary"].asString());
    ...
    plexItem->GetVideoInfoTag()->SetPlayCount(watchedEpisodes >= plexItem->GetVideoInfoTag()->m_iEpisode ? 1 : 0);
    ...
    if (watchedEpisodes >= plexItem->GetVideoInfoTag()->m_iEpisode)
      plexItem->SetOverlayImage(CGUIListItem::ICON_OVERLAY_UNWATCHED);
  ...
  if (!items.IsEmpty())
  {
    ...
    { // flatten if one season or flatten always
      ...
      for(int i = 0; i < items.Size(); i++)
      {
        ...
        XFILE::CDirectory::GetDirectory("plex://tvshows/seasons/" + Base64URL::Encode(curl1.Get()), temp, XFILE::CDirectory::CHints());
  ...
  for (auto variantTrackIt = variantTrack.begin_array(); variantTrackIt != variantTrack.end_array(); ++variantTrackIt)
  {
    ...
    if (!item["Media"].isNull())
    {
      ...
      if (!part.isNull())
      {
        ...
        plexItem->GetMusicInfoTag()->SetType(MediaTypeSong);
...
//  items.SetProperty("library.filter", "true");
  items.GetMusicInfoTag()->SetType(MediaTypeSong);
  SetPlexItemProperties(items);
...
bool CPlexUtils::ParsePlexArtistsAlbum(CFileItemList &items, const CURL &url, const CVariant &directory, bool album)
{
  ...
  for (auto variantIt = variantDirectory.begin_array(); variantIt != variantDirectory.end_array(); ++variantIt)
  {
    ...
    plexItem->GetMusicInfoTag()->SetType(strMediaType);
    plexItem->GetMusicInfoTag()->SetTitle(item["title"].asString());
    if (album)
    {
