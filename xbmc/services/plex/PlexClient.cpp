...
/*
    m_clientSync = new CPlexClientSync(IsOwned(), m_serverName, url.GetWithoutFilename(),
      CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_UUID).c_str(), m_accessToken);
    m_clientSync->Start();
*/
  }
...
{
  //std::unique_lock<CCriticalSection> lock(m_viewItemsLock);
