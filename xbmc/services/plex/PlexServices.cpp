...
public:
  ...
  virtual bool DoWork()
  {
    ...
    {
      CLog::Log(LOGINFO, "CPlexServiceJob: UpdateLibraries");
    ...
    else if (m_function == "FoundNewClient")
    {
      ...
      data["uuid"] = m_strUUID;
      CServiceBroker::GetAnnouncementManager()->Announce(ANNOUNCEMENT::VideoLibrary, "xbmc", "UpdateRecentlyAdded",data);
      CServiceBroker::GetAnnouncementManager()->Announce(ANNOUNCEMENT::AudioLibrary, "xbmc", "UpdateRecentlyAdded",data);
    }
    return true;
  }
...
, m_hasClients(false)
{
  ...
  // (MrMC note: Kodi 21 removed the CURL static redaction registry; tokens are
  //  redacted via CURL::GetRedacted for user details only.)
...
void CPlexServices::Stop()
{
  ...
  if (IsRunning())
  {
    m_bStop = true;
    {
      ...
      std::unique_lock<CCriticalSection> plextvLock(m_plextvCritical);
      if (m_plextv)
        m_plextv->Cancel();
    }
    m_processSleep.Set();
  ...
  std::unique_lock<CCriticalSection> lock2(m_criticalClients);
  m_clients.clear();
  m_gdmListener = nullptr;
  m_updateMins = 0;
  m_playState = MediaServicesPlayerState::stopped;
...
{
  return (!CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_PLEXHOMEUSER).empty() ||
           CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_SERVICES_PLEXGDMSERVER));
}
...
{
  std::unique_lock<CCriticalSection> lock(m_criticalClients);
  clients = m_clients;
}
...
void CPlexServices::OnSettingAction(const std::shared_ptr<const CSetting>& setting)
{
  if (setting == nullptr)
    return;
  ...
  {
    if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_PLEXSIGNIN) == strSignIn)
    {
      // prompt is 'sign-in'
      std::string user;
      std::string pass;
      ...
      if (CGUIDialogLockSettings::ShowAndGetUserAndPassword(user, pass, module, &saveDetails))
      {
        if (!user.empty() && !pass.empty())
        {
          if (GetPlexToken(user, pass))
          {
            ...
            CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_PLEXSIGNIN, strSignOut);
            CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_PLEXHOMEUSER, m_myHomeUser);
            CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_PLEXHOMEUSERTHUMB, m_myHomeUserThumb);
    ...
    else
    {
      ...
      m_authToken.clear();
      CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_PLEXSIGNIN, strSignIn);
      CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_PLEXHOMEUSER, "");
      CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_PLEXHOMEUSERTHUMB, "");
      CLog::Log(LOGDEBUG, "CPlexServices:OnSettingAction sign-out ok");
      std::string serverType = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_GENERAL_SERVER_TYPE);
      if (serverType == "plex")
      {
        CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_GENERAL_SERVER_TYPE,"");
        CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_GENERAL_SERVER_UUID,"");
  ...
  else if (settingId == CSettings::SETTING_SERVICES_PLEXHOMEUSER)
  {
    ...
    if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_PLEXSIGNIN) == strSignOut ||
        CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_PLEXSIGNINPIN) == strSignOut)
    {
...
void CPlexServices::InitiateSignIn()
{
  ...
  if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_PLEXSIGNINPIN) == strSignIn)
  {
    ...
    {
      // change prompt to 'sign-out'
      CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_PLEXSIGNINPIN, strSignOut);
      CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_PLEXHOMEUSER, m_myHomeUser);
      CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_PLEXHOMEUSERTHUMB, m_myHomeUserThumb);
  ...
  else
  {
    ...
    // clear authToken and change prompt to 'sign-in'
    m_authToken.clear();
    CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_PLEXSIGNINPIN, strSignIn);
    CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_PLEXHOMEUSER, "");
    CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_PLEXHOMEUSERTHUMB, "");
    CLog::Log(LOGDEBUG, "CPlexServices:OnSettingAction sign-out ok");
    std::string serverType = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_GENERAL_SERVER_TYPE);
...
void CPlexServices::Announce(AnnouncementFlag flag, const std::string& sender, const std::string& message, const CVariant& data)
{
  if ((flag & AnnouncementFlag::Player) && sender == "xbmc")
  {
    using namespace StringHasher;
    switch(mkhash(message.c_str()))
    {
      case "OnPlay"_mkhash:
  ...
  else if ((flag & AnnouncementFlag::Other) && sender == "plex")
  {
    if (message == "UpdateLibrary")
    {
      AddJob(new CPlexServiceJob(0, "UpdateLibraries"));
    }
    else if (message == "ReloadProfiles")
    {
...
void CPlexServices::OnSettingChanged(const std::shared_ptr<const CSetting>& setting)
{
  ...
  */
  ...
  {
    m_useGDMServer = CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_SERVICES_PLEXGDMSERVER);
  ...
  else if (settingId == CSettings::SETTING_SERVICES_PLEXUPDATEMINS)
  {
    ...
    m_updateMins = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_PLEXUPDATEMINS);
    if (IsRunning())
    {
...
{
  CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_PLEXMYPLEXAUTH, m_authToken);
  CServiceBroker::GetSettingsComponent()->GetSettings()->Save();
}

void CPlexServices::GetUserSettings()
{
  m_authToken  = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_PLEXMYPLEXAUTH);
  m_updateMins  = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_PLEXUPDATEMINS);
  m_useGDMServer = CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_SERVICES_PLEXGDMSERVER);
  m_myHomeUser = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_PLEXHOMEUSER);
  m_myHomeUserThumb = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_PLEXHOMEUSERTHUMB);
}
...
void CPlexServices::Process()
{
  ...
  while (!m_bStop)
  {
    if (g_application.IsInitialized() && g_application.IsAppFocused())
    {
      m_processSleep.Wait(std::chrono::milliseconds(750));
      m_processSleep.Reset();
      if (CServiceBroker::GetGUI()->GetWindowManager().GetActiveWindow() != WINDOW_SPLASH)
        break;
    }
    m_processSleep.Wait(std::chrono::milliseconds(250));
    m_processSleep.Reset();
  }
  ...
  while (!m_bStop)
  {
    ...
    else
    {
      ...
      if (iface && iface->IsConnected())
      {
        ...
        if (router != INADDR_NONE && CServiceBroker::GetNetwork().PingHost(router, 0, 1000))
        {
    ...
    if (CDNSNameCache::Lookup("plex.com", ip))
    {
      ...
      if (CServiceBroker::GetNetwork().PingHost(plexdotcom, 0, 1000))
      {
  ...
  if (m_gdmListener)
  {
    ...
    SOCKETS::CUDPSocket *socket = (SOCKETS::CUDPSocket*)m_gdmListener->GetFirstReadySocket();
...

  {
    ...
    std::unique_lock<CCriticalSection> plextvLock(m_plextvCritical);
    delete m_plextv; m_plextv = nullptr;
  }
...
void CPlexServices::CheckForGDMServers()
{
  if (m_useGDMServer)
  {
    ...
    if (socket)
    {
      SOCKETS::CAddress discoverAddress(NS_BROADCAST_ADDR.c_str());
      discoverAddress.saddr.saddr4.sin_port = htons(NS_PLEX_MEDIA_SERVER_PORT);
...
bool CPlexServices::AddClient(CPlexClientPtr foundClient)
{
  ...
  {
    std::string uuid = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_GENERAL_SERVER_UUID);
    if (uuid == foundClient->GetUuid())
  ...
  if (plex.Get(url.Get(), strResponse))
  {
    ...
    if (MediaContainer)
    {
      ...
      if (atoi(users.c_str()) == 1)
      {
        ...
        CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_GENERAL_SERVER_TYPE,"plex");
        CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_GENERAL_SERVER_UUID,"");
        CServiceBroker::GetSettingsComponent()->GetSettings()->Save();
        return true;
      }
      else if (atoi(users.c_str()) > 1)
      {
        ...
        while (UserNode)
        {
          ...
          plexUser->SetArt("icon", XMLUtils::GetAttribute(UserNode, "thumb") + "&s=512");
