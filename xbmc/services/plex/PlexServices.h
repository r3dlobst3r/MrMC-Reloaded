...
public:
  ...
  // ISettingCallback
  virtual void OnSettingAction(const std::shared_ptr<const CSetting>& setting) override;
  virtual void OnSettingChanged(const std::shared_ptr<const CSetting>& setting) override;

  // IAnnouncer callbacks
  virtual void Announce(ANNOUNCEMENT::AnnouncementFlag flag, const std::string& sender, const std::string& message, const CVariant& data) override;
...
private:
  ...
  mutable CCriticalSection m_criticalClients;
