...
{
  return !CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_TRAKTACESSTOKEN).empty();
}

void CTraktServices::Announce(AnnouncementFlag flag, const std::string& sender, const std::string& message, const CVariant& data)
{
  if (!IsEnabled())
    return;
  ...
  if ((flag & AnnouncementFlag::Player) && sender == "xbmc")
  {
    ...
    switch(mkhash(message.c_str()))
    {
      ...
      case "OnPlay"_mkhash:
        ...
        {
          percentage = 0;
          if (item.GetVideoInfoTag()->GetResumePoint().totalTimeInSeconds > 0)
            percentage = 100.0 * item.GetVideoInfoTag()->GetResumePoint().timeInSeconds / item.GetVideoInfoTag()->GetResumePoint().totalTimeInSeconds;
        }
        if (percentage < 0.0)
          percentage = 0.0;
  ...
  else if ((flag & AnnouncementFlag::GUI) && sender == "xbmc")
  {
    if (std::string(message) == "OnCreated")
    {
      GetUserSettings();
      CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_TRAKTPULLWATCHED, "Idle");
      CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_TRAKTPUSHWATCHED, "Idle");
      m_canSync = IsEnabled();
    }
  }
  else if ((flag & AnnouncementFlag::Other) && sender == "trakt")
  {
    if (message == "ReloadProfiles")
    {
      // restart if MrMC profiles has changed
      GetUserSettings();
    }
  }
  else if ((flag & AnnouncementFlag::VideoLibrary) && sender == "xbmc")
  {
    if (message == "OnScanFinished" && m_canSync)
    {
      int traktSetting = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_TRAKTACTIONAFTERLIBRARYUPDATE);
      if (traktSetting == 1)
      {
        m_canSync = false;
...
void CTraktServices::OnSettingAction(const std::shared_ptr<const CSetting>& setting)
{
  /*
  ...
  {
    if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_TRAKTSIGNINPIN) == strSignIn)
    {
      if (GetSignInPinCode())
      {
        // change prompt to 'sign-out'
        CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_TRAKTSIGNINPIN, strSignOut);
    ...
    else
    {
      ...
      CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_TRAKTSIGNINPIN, strSignIn);
           ...
           settingId == CSettings::SETTING_SERVICES_TRAKTPUSHWATCHED)
  {
    ...
    {
      m_canSync = false;
      CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_TRAKTPULLWATCHED, "Running");
      AddJob(new CTraktServiceJob("TraktPullWatched"));
    }
    else
    {
      m_canSync = false;
      CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_TRAKTPUSHWATCHED, "Running");
      AddJob(new CTraktServiceJob("TraktPushWatched"));
    }
  }
}

void CTraktServices::OnSettingChanged(const std::shared_ptr<const CSetting>& setting)
{
...
{
  CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_TRAKTACESSTOKEN, m_authToken);
  CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_TRAKTACESSREFRESHTOKEN, m_refreshAuthToken);
  CServiceBroker::GetSettingsComponent()->GetSettings()->SetInt(CSettings::SETTING_SERVICES_TRAKTACESSTOKENVALIDITY, m_authTokenValidity);
  CServiceBroker::GetSettingsComponent()->GetSettings()->Save();
}

void CTraktServices::GetUserSettings()
{
  m_authToken  = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_TRAKTACESSTOKEN);
  m_refreshAuthToken  = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_TRAKTACESSREFRESHTOKEN);
  m_authTokenValidity  = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_TRAKTACESSTOKENVALIDITY);
}
...
const MediaServicesPlayerState CTraktServices::GetPlayState(CFileItem &item)
{
  ...
  std::unique_lock<CCriticalSection> lock(m_playStatesLock);
...
void CTraktServices::SetItemWatchedJob(CFileItem &item, bool watched, bool setLastWatched)
{
  ...
  if (item.HasVideoInfoTag() && item.GetVideoInfoTag()->m_type == MediaTypeMovie)
  {
    ...
    movie["title"]      = item.GetVideoInfoTag()->GetTitle();
...
void CTraktServices::ReportProgress(CFileItem &item, const std::string &status, double percentage)
{
  ...
  if (!status.empty())
  {
    ...
    {
      data["movie"]["title"] = item.GetVideoInfoTag()->GetTitle();
  ...
  if (movieItems.Size() > 0)
  {
    ...
    for (int i = 0; i < movieItems.Size(); ++i)
    {
      ...
      if (!CompareMovieToHistory(*movieItems[i], watchedMovies))
      {
        ...
        CVariant movie;
        movie["title"]      = movieItems[i]->GetVideoInfoTag()->GetTitle();
...
CVariant CTraktServices::GetTraktCVariant(const std::string &url)
{
  ...
  curlfile.SetRequestHeader("Authorization", "Bearer " + CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_TRAKTACESSTOKEN));
...
void CTraktServices::RefreshAccessToken()
{
  ...
  CVariant data;
  data["refresh_token"] = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_TRAKTACESSREFRESHTOKEN);
  data["client_id"] = NS_TRAKT_CLIENTID;
  data["client_secret"] = NS_TRAKT_CLIENTSECRET;
...
void CTraktServices::CheckAccessToken()
{
  ...
  time_t tNow = 0;
  time_t tExpiry = (time_t)CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_TRAKTACESSTOKENVALIDITY);
  now.GetAsTime(tNow);
  if (tNow > tExpiry)
  {
    RefreshAccessToken();
