  ...
  switch (status)
  {
    case STATUS_PLAY:
      m_bridge->getLight(lightid)->setBrightness(
            uint8_t(CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_DIMBRIGHT) * 255 / 100),
            uint32_t(CServiceBroker::GetSettingsComponent()->GetSettings()->GetNumber(CSettings::SETTING_SERVICES_HUE_DIMDUR) * 1000.0)
            );
      break;
    case STATUS_PAUSE:
      if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_SERVICES_HUE_DIMOVERPAUSEDBRIGHT))
        m_bridge->getLight(lightid)->setBrightness(
              uint8_t(CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_DIMPAUSEDBRIGHT) * 255 / 100),
              uint32_t(CServiceBroker::GetSettingsComponent()->GetSettings()->GetNumber(CSettings::SETTING_SERVICES_HUE_DIMDUR) * 1000.0)
              );
      else
        RevertLight(lightid);
      break;
    case STATUS_STOP:
      if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_SERVICES_HUE_DIMOVERUNBRIGHT))
        m_bridge->getLight(lightid)->setBrightness(
              uint8_t(CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_DIMUNBRIGHT) * 255 / 100),
              uint32_t(CServiceBroker::GetSettingsComponent()->GetSettings()->GetNumber(CSettings::SETTING_SERVICES_HUE_DIMDUR) * 1000.0)
              );
      else
        RevertLight(lightid);
...
void CHueServices::DimScene(int status)
{
  ...
  switch (status)
  {
    case STATUS_PLAY:
    {
      std::string sceneId = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_HUE_DIMSCENE);
    ...
    case STATUS_PAUSE:
      ...
      if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_SERVICES_HUE_DIMOVERPAUSEDBRIGHT))
        m_bridge->setScene(
              CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_HUE_DIMPAUSEDSCENE),
              uint32_t(CServiceBroker::GetSettingsComponent()->GetSettings()->GetNumber(CSettings::SETTING_SERVICES_HUE_DIMDUR) * 1000.0)
              );
      else
      {
    ...
    case STATUS_STOP:
      ...
      if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_SERVICES_HUE_DIMOVERUNBRIGHT))
        m_bridge->setScene(
              CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_HUE_DIMSTOPPEDSCENE),
              uint32_t(CServiceBroker::GetSettingsComponent()->GetSettings()->GetNumber(CSettings::SETTING_SERVICES_HUE_DIMDUR) * 1000.0)
              );
      else
      {
...
void CHueServices::Announce(AnnouncementFlag flag, const std::string& sender, const std::string& message, const CVariant& data)
{
...
void CHueServices::Start()
{
  ...
  if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_SERVICES_HUE_ENABLE))
  {
    if (IsRunning())
      StopThread();
    CThread::Create();
...
void CHueServices::SettingOptionsHueStreamGroupsFiller(const CSetting *setting, std::vector< std::pair<std::string, int> > &list, int &current, void *data)
{
  ...
  {
    std::unique_lock<CCriticalSection> lock(s_settings_critical);

    std::string ip = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_HUE_IP);
    std::string username = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_HUE_USERNAME);
    std::string clientkey = CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_HUE_CLIENTKEY);
    ...
    if (!ip.empty() && !username.empty())
    {
      ...
      if (CServiceBroker::GetNetwork().PingHost(bridgeip, 0, 1000))
      {
...
void CHueServices::OnSettingAction(const std::shared_ptr<const CSetting>& setting)
{
  if (setting == nullptr)
    return;
  ...
  {
    if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetString(CSettings::SETTING_SERVICES_HUE_DISCOVER) == strSignIn)
      SignIn();
    else
      SignOut();
  }
...
void CHueServices::OnSettingChanged(const std::shared_ptr<const CSetting>& setting)
{
  if (setting == NULL)
    return;
...
void CHueServices::Process()
{
  ...
  int streamgroup = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_STREAMGROUPID);
  captureNeeded |= (streamgroup > 0);

  if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_LIGHT1ID) > 0)
  {
    captureNeeded |= (CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_LIGHT1MODE) == MODE_COLOR);
  }
  if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_LIGHT2ID) > 0)
  {
    captureNeeded |= (CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_LIGHT2MODE) == MODE_COLOR);
  }
  if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_LIGHT3ID) > 0)
  {
    captureNeeded |= (CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_LIGHT3MODE) == MODE_COLOR);
  }
  ...
  while (!m_bStop)
  {
    ...
    if (curstatus != STATUS_STOP || m_continuous)
    {
      if (!m_bridge)
      {
        ...
        bool forceOn = CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_SERVICES_HUE_FORCEON);
        bool forceOnAfterSunset = CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_SERVICES_HUE_FORCEONAFTERSUNSET);
      ...
      if (captureNeeded && !provider)
      {
        ...
        minL = float(CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_MINBRIGHT)) / 100.0f;
        maxL = float(CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_MAXBRIGHT)) / 100.0f;
        biasC = float(((100 - CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_COLORBIAS)) / 5)+1) * 0.0011f;
      }
...
bool CHueServices::SignIn()
{
  ...
  CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_HUE_IP, curBridge.getIp());
  CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_HUE_USERNAME, curBridge.getUsername());
  CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_HUE_CLIENTKEY, curBridge.getClientkey());
...
bool CHueServices::SignOut()
{
  ...
  CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_HUE_USERNAME, "");

  CServiceBroker::GetSettingsComponent()->GetSettings()->SetString(CSettings::SETTING_SERVICES_HUE_DISCOVER, strSignIn);
...
bool CHueServices::InitConnection()
{
  ...
  if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_LIGHT1ID) > 0)
  {
    size_t id = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_LIGHT1ID);
  ...
  if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_LIGHT2ID) > 0)
  {
    size_t id = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_LIGHT2ID);
    if (lights.find(id) != lights.end())
    {
  ...
  if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_LIGHT3ID) > 0)
  {
    size_t id = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_LIGHT3ID);
    if (lights.find(id) != lights.end())
    {
  ...
  if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_LIGHT4ID) > 0)
  {
    size_t id = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_SERVICES_HUE_LIGHT4ID);
    if (lights.find(id) != lights.end())
    {
